...
SELECT EMP_NAME AS "직원명", EMAIL "이메일", PHONE AS 전화번호, HIRE_DATE 고용일
FROM EMPLOYEE;
...
FROM EMPLOYEE
-- WHERE SALARY >= 6000000 AND SALARY <= 10000000;
...
-- * 별칭은 WHERE절이 실행된 이후에 부여된 것이므로 WHERE절에서는 사용할 수 없음!!
...
WHERE EMAIL LIKE '____\_%' ESCAPE '\'       -- 메일주소 '_'의 앞이 4자
    AND DEPT_CODE IN ('D9', 'D6')           -- DEPT_CODE가 D9 또는 D6
    AND HIRE_DATE BETWEEN '90/01/01' AND '00/12/01' -- 고용일이 90/01/01 ~ 00/12/01
    AND SALARY >= 2700000;                  -- 급여가 270만 이상
