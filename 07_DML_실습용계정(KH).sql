...
--------------------------------------------------------------------------------
/*
    * DML (데이터 조작 언어) : 테이블에 데이터(값)를 추가하거나 (INSERT),
                                                변경하거나 (UPDATE),
...
VALUES (900, '장현우', '880709-1000000', 'hw000@kh.or.kr', NULL, 'D4', 'J4', 4000000, 0.3, NULL, SYSDATE, NULL, 'N');
...
*/
-- EMPLOYEE 테이블에 데이터 추가. 직원번호, 이름, 주민번호, 이메일, 직급 코드만 가지고 추가.
INSERT INTO EMPLOYEE (EMP_ID, EMP_NO, EMP_NAME, EMAIL, JOB_CODE)
            VALUES (901, '040709-4000000', '공윤정', 'yj04@kh.or.kr', 'J6');
...
SELECT SALARY, BONUS 
FROM EMP_TABLE
WHERE EMP_NAME = '방명수';  -- 1380000	NULL
...
UPDATE EMP_TABLE
 ...
 SELECT * FROM EMP_TABLE WHERE EMP_NAME = '윤은해'; -- 2200000	NULL --> 3740000	0.2

-- ASIA 지역에서 근무중인 직원들의 보너스를 0.3으로 변경
--   변경할 내용 => 보너스를 0.3으로 변경
--   조건 => ASIA 지역에서 근무중인 직원 ( ASIA 지역코드에 해당하는 부서에 속한 직원 )
...
/*
    * DELETE : 테이블에 저장되어 있는 데이터를 삭제할 때 사용하는 구문
    ...
    => WHERE 절 생략 시 테이블의 모든 데이터(행)가 삭제됨!! * 주의 *
*/
...
SELECT * FROM EMP_TABLE WHERE EMP_NAME = '장현우';
